...
public class NoteServiceImp implements NoteService, BaseService<Note> {
	...
	private static Logger log = Logger.getLogger(NoteServiceImp.class);
	...
	public List<Note> findAll() {
		...
		try {
			...
			noteList = (List<Note>) RedisDao.getListBean(sortKey, Note.class, jedis);

			// dubbo 调用的时候防止java.sql.Blob cannot be assigned from null
			// ，也就是blob字段不能为空
			delalBlob(noteList);
		} catch (Exception e) {
	...
	public Note queryById(String i) {
		...
		// 查询不用开启事物
		RedisDao rd = new RedisDao(jedis);
		note = (Note) RedisDao.getBean("Note:" + i, note.getClass(), jedis);
		pool.returnResource(jedis);
